...
def detectGasLeakage() -> str:
    ...
    api_url = f"https://glowing-tribble-wxqrpj9457xh5jvj-8000.app.github.dev/predict-leakage?temperature={temperature}&humidity={humidity}&lpg={lpg}"
    ...
    try:
        ...
        print(f"An error occurred: failed with status code")
        return leakage_value
...
while True:
    ...
    if iterations % 4 == 0 :
        if data["sms_enabled"] == True and data["leakage"] == "yes":
            try:
                print("Sneding SMS")
                sendSMS()
